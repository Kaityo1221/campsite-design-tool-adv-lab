(() => {
  ...
  function validPoint(point) {
    ...
    return {
      ...
      isTarget:
        typeof point.isTarget === 'boolean'
          ? point.isTarget
          : Boolean(point.isAdded || point.isExisting),
  ...
  function buildDensityFacts(points) {
    ...
    const existing = points.filter(
      point => point.isExisting
    );
    ...
    return added.map(center => {
      ...
      return {
        ...
        existingCount:
          nearbyExisting.length,

        threshold:
          DENSITY_MIN_EXISTING,
    ...
    const activeWarnings = (
      input.warnings?.active ||
      input.activeWarnings ||
      []
    ).map(normalizeWarning);

    const referenceWarnings = (
      input.warnings?.reference ||
      input.referenceWarnings ||
      []
    ).map((warning, index) =>
      normalizeWarning(
        {
    ...
    return {
      ...
      localRules:
        Array.isArray(input.localRules)
          ? [...input.localRules]
          : [],
      ...
      stats: {
        ...
        activeDistanceWarningCount:
          activeWarnings.length,

        referenceDistanceWarningCount:
          referenceWarnings.length,

        densityCandidateCount:
          densityTriggered.length,

        densitySupportCandidateCount:
          supportTriggered.length
      }
    };
  }
