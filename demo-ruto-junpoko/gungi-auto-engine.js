(() => {
  ...
  const EVENT_DEFS = {
    DENSITY_01: {
      id: 'DENSITY_01', title: '密集地点', type: '対立型', priority: 50,
      cuts: [
    ...
    DENSITY_REST_01: {
      id: 'DENSITY_REST_01', title: '密集＋休憩・利便', type: '補完型', priority: 60,
      cuts: [
    ...
    ENTRANCE_01: {
      id: 'ENTRANCE_01', title: '入口・集合導線', type: '対立型', priority: 80,
      cuts: [
    ...
    LOOP_01: {
      id: 'LOOP_01', title: '回遊導線', type: '補完型', priority: 40,
      cuts: [
    ...
    NARROW_PATH_01: {
      id: 'NARROW_PATH_01', title: '狭路・橋・木道', type: '確認型', priority: 70,
      cuts: [
    ...
    PARKING_01: {
      id: 'PARKING_01', title: '駐車場・車両動線', type: '確認型', priority: 70,
      cuts: [
  ...
  function normalizePoint(point, index) {
    ...
    return { ...point, id: point.id || `p${index + 1}`, isAdded, isExisting, isSupport, _text: text };
  }

  function eventResult(id, payload = {}) {
    const def = EVENT_DEFS[id];
    return { ...def, ...payload, cuts: def.cuts.map(cut => ({ ...cut })) };
  }
  ...
  function keywordEvent(points, id, regex, options = {}) {
    ...
    return eventResult(id, {
      ...
      center: candidates[0], matchedPoints: candidates,
      metrics: { matchedCount: candidates.length }
    });
  }

  function detectDensityFromFacts(facts) {
    const factData = facts?.facts || facts || {};
    const densityFacts = Array.isArray(factData.density)
      ? factData.density.filter(fact => fact.triggered).sort((a,b)=>b.existingCount-a.existingCount)
      : [];
    if (!densityFacts.length) return [];
    const supportFacts = Array.isArray(factData.support) ? factData.support : [];

    return densityFacts.map(densityFact => {
      const supportFact = supportFacts.find(fact => fact.triggered && fact.center?.id === densityFact.center?.id) || null;
      const nearbyExisting = Array.isArray(densityFact.nearbyExisting) ? densityFact.nearbyExisting : [];
      const nearbySupport = Array.isArray(supportFact?.nearbySupport) ? supportFact.nearbySupport : [];
      const common = {
        candidateId: `density:${densityFact.center?.id}`,
        confidence: densityFact.confidence || 'confirmed',
        center: densityFact.center,
        matchedPoints: [densityFact.center, ...nearbyExisting.map(item => item.point)].filter(Boolean),
        supportPoints: nearbySupport.map(item => item.point).filter(Boolean),
        metrics: {
          radiusM: densityFact.radiusM,
          nearbyExistingCount: densityFact.existingCount,
          supportCount: supportFact?.supportCount || 0
        }
      };
      if (supportFact?.triggered) {
        return eventResult('DENSITY_REST_01', {
          ...common,
          reason: `Fact Layer: ${densityFact.center?.name || '追加POI'}の${densityFact.radiusM}m以内に既存POIが${densityFact.existingCount}件。さらに休憩・利便施設が${supportFact.supportCount}件`
        });
      }
      return eventResult('DENSITY_01', {
        ...common,
        reason: `Fact Layer: ${densityFact.center?.name || '追加POI'}の${densityFact.radiusM}m以内に既存POIが${densityFact.existingCount}件`
      });
    });
  }
  ...
  function detectAll(input = {}) {
    ...
    const found = [];
    const parking = keywordEvent(points, 'PARKING_01', RX.parking, { targetOnly: true, confidence: 'medium', reason: '名称またはフォルダ名から駐車場・ロータリー・車両に関係するPOIを検出しました。' });
    if (parking) found.push(parking);
    const narrow = keywordEvent(points, 'NARROW_PATH_01', RX.narrow, { targetOnly: true, confidence: 'medium', reason: '名称またはフォルダ名から狭路・橋・木道・自転車道に関係するPOIを検出しました。' });
    if (narrow) found.push(narrow);
    const entrance = keywordEvent(points, 'ENTRANCE_01', RX.entrance, { addedOnly: true, confidence: 'medium', reason: '追加POIの名称またはフォルダ名から入口・ゲート・改札に関係するPOIを検出しました' });
    if (entrance) found.push(entrance);
    const loop = keywordEvent(points, 'LOOP_01', RX.loop, { targetOnly: true, minCount: 2, confidence: 'medium', reason: '周回・遊歩道・園路を示す候補が複数あるため回遊導線候補として検出' });
